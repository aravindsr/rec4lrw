...
public class task2_extracttopicbyuser {
		...
		int expectedfilescount=0;
		    ...
		    for (int i = 0; i < listOfFiles.length; i++) {
		      if (listOfFiles[i].isFile()) {
		    	if(listOfFiles[i].getName().contains("rar_ind") || listOfFiles[i].getName().contains(".txt") || listOfFiles[i].getName().contains("task1")|| listOfFiles[i].getName().contains("task3"))
		    	{
		    		//do nothing
		    	}
		    	else
		    	{
		    		...
		    		String researchtopic=breakstring[1];
